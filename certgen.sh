...
openssl req -subj "/C=US/ST=Random/L=Random/O=Global Security/OU=IT Department/CN=localhost"  -new -sha256 -nodes -out fluent-bit.csr -newkey rsa:2048 -keyout fluent-bit.key
openssl x509 -req -passin pass:"$1" -in fluent-bit.csr -CA rootCA.pem -CAkey rootCA.key -CAcreateserial -out fluent-bit.crt -days 500 -sha256 -extfile  <(printf "subjectAltName=DNS:localhost,DNS:fluent-bit")
